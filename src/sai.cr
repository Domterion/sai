...
module Sai
  ...
  post "/api/add" do |env|
    begin
      ...
      next {msg: "Invalid Meeting ID provided."}.to_json
    end

    if !id.nil?
      if id > 100_000_000_000 || DATABASE.meetings.fetch(id) { nil }
        next {msg: "Invalid Meeting ID provided, it may already be in our database."}.to_json
      end
    end
    ...
    begin
      ...
      if !passcode.nil?
        if passcode.as(String).size > 16
          next {msg: "Invalid passcode."}.to_json
        end
      end
    rescue
      passcode = nil
